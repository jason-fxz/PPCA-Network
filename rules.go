...
func InitRules(filename string) {
    ...
    for scanner.Scan() {
        ...
        if strings.HasPrefix(line, "/") && strings.HasSuffix(line, "/") {
			...
			rules = append(rules, proxyrule{0x00, pattern, method})
			...
			// CIDR IP 段
			rules = append(rules, proxyrule{0x01, line, method})
        }
    }
...
func Match(url string) (string, error) {
	for _, rule := range rules {
		if rule.typ == 0x00 { // use regex
			...
			flag, err := isIPInCIDR(url, rule.pattern)
			if err != nil {
				// log.Println(err)
				continue
			}
